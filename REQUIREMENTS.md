...
*   **REQ-029 (Graceful Teardown):** Upon receiving a SIGTERM or SIGINT signal, the daemon must gracefully flush all priority scheduler queues, close active WebSocket connections, disconnect from the NATS broker, and terminate the process with exit code 0.
