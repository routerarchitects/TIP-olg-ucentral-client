...
func TestTCRM018_DispatchTimer_StopVsFireRace(t *testing.T) {
	...
	m, _ := NewRequestManager(1*time.Hour, config, cache, scheduler, store, 1000, 15*time.Minute, 100)
...
func TestTCRM020_DuplicateIDRejectedAfterCompletion(t *testing.T) {
	...
	_, err = m.CreateTransaction("session-1", cloudRPCID, true, "ping", 10*time.Second, false)
	var cachedErr *CachedResponseError
...
func TestTCRM023_CapacityExceeded(t *testing.T) {
	...
	m, _ := NewRequestManager(10*time.Second, config, cache, scheduler, store, 3, 15*time.Minute, 100)
